...
class UserRegistrationView(viewsets.ViewSet):
    ...
    def _activate_get(self, request, user, *args, **kwargs):
        return render(request, 'cycling_org/email/user_activation_confirm.html', context={'new_user': user})
    ...
    def _recover_password_get(self, request, user, *args, **kwargs):
        return render(request, 'cycling_org/email/user_recover_password_confirm.html', context={'new_user': user})
...
class MemberView(viewsets.ModelViewSet):
    ...
    def send_email_verify_code(self, request, *args, **kwargs):
        ...
        message = render_to_string('cycling_org/email/member_verify_email.html', {
            'member': me,
            'code': code,
            'expiry': expiry,
            'request': self.request
    ...
    def send_phone_verify_code(self, request, *args, **kwargs):
        ...
        expiry = settings.OTP_MEMBER_VERIFY_INTERVAL
        message = render_to_string('cycling_org/sms/member_verify_phone.html', {
            'member': me,
            'code': code,
            'expiry': expiry,
            'request': self.request
...
class OrganizationView(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if global_pref.get('core_backend__disabled_create_org') and \
                not self.request.user.has_perm('cycling_org.add_organization'):
...
class FieldsTrackingView(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(content_type__app_label='cycling_org')
